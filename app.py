...
def get_posts(of_user = ''):
    ...
    if of_user != '':
        ...
        cursor.execute("""select name, posts.user_id, likes, image,photo from posts INNER JOIN users on posts.user_id = users.user_id where posts.user_id = '{}' order by posts.id desc""".format(of_user))
    else:
        ...
        cursor.execute("""select name, posts.user_id, likes, image, photo from posts INNER JOIN users on posts.user_id = users.user_id order by posts.id desc""")
...
def new_post():
    if 'user_id' in session:
        ...
        if f.filename is not '':
            ...
            f.save(filename)
            print("""INSERT INTO `posts` (`user_id`, `image`) values ("{}","{}")""".format(user_id, filename))
            cursor.execute("INSERT INTO posts ( user_id, image) VALUES (%s, %s)", ( user_id, filename))
        else:
            print("""INSERT INTO `posts` ( `user_id`) values ("{}")""".format( user_id))
            cursor.execute("""INSERT INTO `posts` ( `user_id`) values ("{}")""".format( user_id))
        mysql.connection.commit()
        return redirect('/')
    else:
        return render_template('login.html')
